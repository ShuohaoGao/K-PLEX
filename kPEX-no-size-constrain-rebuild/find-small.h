...
public:
    ...
    void start_search()
    {
        ...
        {
            printf("now given lb= %d ,focus on lb+1\n", lb);
            ...
            pstart = g.pstart;
            edge_to = g.edge_to;
            IE_framework(); // start branch-and-bound
        }
    }
    ...
    {
        if (solution.size() > lb) // we find the final solution!
            return;
    ...
    bool fast_reduce(set<ui> &S, set<ui> &C, vector<ui> &removed_vertics)
    {
        ...
        if (S.size() >= k)
        {
            ...
            for (ui u : S)
            {
                assert(neighbor_cnt_in_S[u] + k >= S.size());
            ...
            }
            {
                ui state = removed_vertics.size();
                for (ui v : C)
                    if (neighbor_cnt_in_S[v] + k < S.size() + 1)
                        removed_vertics.push_back(v);
                for (ui i = state; i < removed_vertics.size(); i++)
                {
                    ui v = removed_vertics[i];
